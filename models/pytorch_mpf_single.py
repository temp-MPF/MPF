...
class MPFSingleModel(Model):
    def __init__(
        ...
        **kwargs,
        ...
        self.logger = get_module_logger("MPF single")
        self.logger.info("MPF single Model...")
    ...
    def train_epoch(self, data_set):
        ...
        for batch in tqdm(data_set, total=max_steps):
            ...
            for i in range(len(self.patterns)):
                x_train_values_new = eval(
                    "data.detach().cpu().numpy()[:,{},:]".format(self.patterns[i])
                )
                x_train_values_list.append(
                    signal.resample(
                        x_train_values_new, self.length_after_resample[i], axis=1
                    )
                )
            batch_x_train_values = [
                torch.tensor(x_train_values_list[j]).float().to(self.device)
                for j in range(self.diff_len_num)
            ]
            hidden = self.model(batch_x_train_values[0])
    ...
    def fit(self, dataset, evals_result=dict()):
        ...
        for epoch in range(self.n_epochs):
            ...
            self.scheduler.step(valid_metrics["IC"])
            new_lr = [group["lr"] for group in self.optimizer.param_groups]
            # todo: stop_rounds=> 0
            flag = False
            for i in range(len(last_lr)):
                if last_lr[i] != new_lr[i]:
                    flag = True
            if flag:
                stop_rounds = -1
                self.logger.info("adjust the lr, {}".format(stop_rounds))
            if self.eval_test:
