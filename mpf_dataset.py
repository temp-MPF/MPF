...
# this code from https://github.com/microsoft/qlib/blob/main/examples/benchmarks/TRA/src/dataset.py
import copy
import torch
import warnings
import numpy as np
...
class MTSDatasetH(DatasetH):
    ...
    def setup_data(self, handler_kwargs: dict = None, **kwargs):
        ...
        if self.memory_mode == "sample":
            self._memory = np.zeros((len(self._data), self.export_num, self.num_states), dtype=np.float32)
        elif self.memory_mode == "daily":
            self._memory = np.zeros((len(self._daily_index), self.export_num, self.num_states), dtype=np.float32)
        else:
    ...
    def __iter__(self):
        ...
        for i in range(len(indices))[::batch_size]:
            ...
            for j in indices[i : i + batch_size]:
                ...
                if self.batch_size < 0:
                    ...
                    if self.memory_mode == "daily":
                        ...
                        state.append(np.stack([_maybe_padding(self._memory[slc,branch_idx], self.seq_len, self._zeros)[: -self.horizon] for branch_idx in range(self.export_num)],axis = 0))
                ...
                for slc in slices_subset:
                    ...
                    if self.memory_mode == "sample":
                        # print ("the size of self._memory[slc,branch_idx] is:{}".format(self._memory[slc].shape))
                        state.append(np.stack([_maybe_padding(self._memory[slc,branch_idx], self.seq_len, self._zeros)[: -self.horizon] for branch_idx in range(self.export_num)],axis = 0))
            ...
            state = _to_tensor(np.stack(state),self.device)
